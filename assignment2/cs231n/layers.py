...
def batchnorm_backward(dout, cache):
    ...
    dsigma = np.sum( dx_gamma * cache['normalized_x'] * (-1/cache['sample_sigma']), axis=0)
    dms_div = dx_gamma / cache['sample_sigma']
    dvar = dsigma / ( cache['sample_sigma'] * 2 )
    dvarmean = dvar/N   #np.tile(dvar, (N,1))/N
    dms_sq = 2 * dvarmean * cache['meanshifted_x']
    dmeanshift = dms_div + dms_sq
    dmean = np.sum(-dmeanshift, axis=0)
    dx_meanshift = dmeanshift
    dx_mean = dmean/N   #np.tile(dmean, (N,1))/N
    dx = dx_meanshift + dx_mean
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    pad = conv_param['pad']
    assert (H + 2 * pad - HH) % stride == 0, 'Conv Stride not compatible with height'
    assert (W + 2 * pad - WW) % stride == 0, 'Conv Stride not compatible with width'
    Ho = 1 + (H + 2 * pad - HH) // stride
    Wo = 1 + (W + 2 * pad - WW) // stride
    out = np.zeros((N, F, Ho, Wo))
    x_pad = np.pad(x, mode='constant', pad_width=((0,0), (0,0), (pad,pad), (pad,pad)))
    for f in range(F):
        for i in range(Ho):
            for j in range(Wo):
                ist, jst = (i*stride, j*stride)
                conv = w[f] * x_pad[..., ist:ist+HH, jst:jst+WW]
                out[:, f, i, j] = np.sum(conv, axis=(1,2,3)) + b[f]
...
def conv_backward_naive(dout, cache):
    ...
    for f in range(F):
        for i in range(Ho):
            for j in range(Wo):
                ist, jst = (i*stride, j*stride)
                dups = dout[:, f, i , j][:, np.newaxis, np.newaxis, np.newaxis]
                dw[f] += np.sum(dups * x_pad[..., ist:ist+HH, jst:jst+WW], axis=0)
                dx_pad[..., ist:ist+HH, jst:jst+WW] += dups * w[f]   
...
def max_pool_forward_naive(x, pool_param):
    ...
    assert (H - pool_height) % stride == 0, 'Pool Stride not compatible with height'
    assert (W - pool_width) % stride == 0, 'Pool Stride not compatible with width'
    ...
    for i in range(Ho):
        for j in range(Wo):
            ist, jst = [i*stride, j*stride]
            out[..., i, j] = np.max(x[..., ist:ist+pool_height, jst:jst+ pool_width], axis=(2,3))
    ...
    for n in range(N):
        for c in range(C):
            for i in range(Ho):
                for j in range(Wo):
                    ist, jst = [i*stride, j*stride]
                    midx = np.argmax(np.squeeze(x[n, c, ist:ist+pool_height, jst:jst+ pool_width]))
                    mi, mj = np.unravel_index(midx, shape = (pool_height, pool_width))
                    dx[n,c ,ist+mi, jst+mj] = dout[n, c, i, j]
